...
@Component({
  selector: 'app-hr-attendance-run',
  templateUrl: './hr-attendance-run.component.html',
  styleUrls: ['./hr-attendance-run.component.scss']
})
export class HrAttendanceRunComponent implements OnInit {
  ...
  getRequestForPageOneOnlyWithFilter(pageNo) {
    ...
    this._attendanceRun.getAttendanceRunListWithFilters(pageNo, this.defaultPaginationSize, this.allowanceName).subscribe((data: any) => {
  ...
  hideViewRunModal() {
    $('#ViewRunModal').modal('hide');
    this.getRequestForPageOneOnlyWithFilter(1);
  }

  showViewRunModal() {
    $('#ViewRunModal').modal('show');
  }
