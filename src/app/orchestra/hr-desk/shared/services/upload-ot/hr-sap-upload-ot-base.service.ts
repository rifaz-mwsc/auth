...
export class HrSapUploadOtBaseService {
  ...
  getHRDeskPendingOtSapUpload() {
    if (environment.devJsonApiConfig.api_json_server === true) {
      // Node Json Server
      // const httpOptions = this.prepareOptions();
      return this.http.get<any>(`${environment.devJsonApiConfig.api_json_server_url}v1/overtime-requests`)
        .pipe(map(res => {
