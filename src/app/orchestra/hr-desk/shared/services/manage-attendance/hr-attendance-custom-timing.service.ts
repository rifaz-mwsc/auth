...
export class HrAttendanceCustomTimingService {
  ...
  getAttendanceCustomTimingListWithFilters(pageNo, pageSize, searchText, plannedDate, hideDelatedItems) {
    const httpOptions = this.prepareOptions();
    const params = '?page=' + pageNo + '&page_size=' + pageSize + '&search=' + searchText + '&planned_date=' + plannedDate + '&hide_deleted_items=' + hideDelatedItems;

    // const params = '?page=' + pageNo + '&page_size=' + pageSize + '&search=' + searchText + '&planned_date=' + searchText + '&hide_deleted_items=' + searchText;


    //  return this._http.get<any>(`${environment.hrDeskApiConfig.api_url}v1/custom-timings` + params, httpOptions)
  ...
  getAttendanceCustomTimingById() {
    ...
    return this._http.get<any>(`${environment.hrDeskApiConfig.api_url}v1/custom-timing` + params, httpOptions)
      .pipe(map(res => {
        // this.accountList = res;
        // localStorage.setItem('hrDeskHolidays', JSON.stringify(res));
        // this.userAccountsSubject.next(accountData);
  ...
  postAttendanceCustomTimingNew(data) {
    ...
    return this._http.post<any>(`${environment.hrDeskApiConfig.api_url}v1/custom-timing`, body, httpOptions)
      .pipe(map(res => {
  ...
  deleteAttendanceCustomTiming(AttendanceCustomTimingId) {
    ...
    return this._http.delete<any>(`${environment.hrDeskApiConfig.api_url}v1/custom-timing` + params, httpOptions)
      .pipe(map(res => {
        // localStorage.setItem('postStaffPortalLeaveRequest', JSON.stringify(res));
        // this.leaveRequestsSubject.next(res);
        return res;
