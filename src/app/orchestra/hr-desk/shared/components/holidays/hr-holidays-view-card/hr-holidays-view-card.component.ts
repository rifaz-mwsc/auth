...
@Component({
  selector: 'app-hr-holidays-view-card',
  templateUrl: './hr-holidays-view-card.component.html',
  styleUrls: ['./hr-holidays-view-card.component.scss']
})
export class HrHolidaysViewCardComponent implements OnInit {
  ...
  @ViewChild('CardViewRemoveModal', { static: false }) CardViewRemoveModal: ElementRef;
  ...
  showRemoveModal(selecteditem) {
    console.log('showRemoveModal', selecteditem);
    this.selectedData = [];
  ...
  // Remove
  postRemoveHoliday(selectedData) {
    console.log('postRemoveHoliday', selectedData);
    this.showModelLoader = true;
    this.hrHolidaysBase.postHRDeskRemoveHoliday(selectedData.holiday_id).subscribe((data: any) => {
