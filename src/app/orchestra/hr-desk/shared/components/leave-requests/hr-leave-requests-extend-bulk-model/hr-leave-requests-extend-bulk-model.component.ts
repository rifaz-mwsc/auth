...
@Component({
  selector: 'app-hr-leave-requests-extend-bulk-model',
  templateUrl: './hr-leave-requests-extend-bulk-model.component.html',
  styleUrls: ['./hr-leave-requests-extend-bulk-model.component.scss']
})
export class HrLeaveRequestsExtendBulkModelComponent implements OnInit {
  ...
  csvOptions = {
    ...
    showLabels: true,
    showTitle: true,
    title: 'Leave Extend Report',
    useBom: true,
    noDownload: false,
    headers: ["Employee", "Leave Status", "Leave Type", "From Date", "To Date", "Status", "Message", "Message Details"]
  };
  ...
  extend() {
    ...
    if (this.newExpiryDate === undefined || this.newExpiryDate == null || this.newExpiryDate.length <= 0 ||
      this.selectedLeaveList === undefined || this.selectedLeaveList == null || this.selectedLeaveList.length <= 0) {
      ...
      this.selectedLeaveList.forEach(selectedArryObj => {
        ...
        this.leaveRequestsBase.postHRDeskExtendLeaveRequests(
          ...
          this.toastr.success('Extented', 'Leave Request Extented', { closeButton: true, timeOut: this.toastrTimeOut, progressBar: true, enableHtml: true });
          this.showModelLoader = false;
          selectedArryObj.leave_request_http_error_status = 200;
          selectedArryObj.leave_request_http_error_message = 'Leave Request Extented';
          selectedArryObj.leave_request_http_error_message = '';
        }, (error: Response | any) => {
        ...
        //   leave_request_employee_details: selectedArryObj.leave_request_employee_details,
        //   leave_request_status_description: selectedArryObj.leave_request_status_description,
        //   leave_request_absence_type_description: selectedArryObj.leave_request_absence_type_description,
  ...
  downloadCSVReport() {
    ...
    new AngularCsv(this.uploadReport, "Leave Extend Report", this.csvOptions);
  }
