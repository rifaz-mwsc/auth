...
@Component({
  selector: 'app-hr-leave-requests-view-table',
  templateUrl: './hr-leave-requests-view-table.component.html',
  styleUrls: ['./hr-leave-requests-view-table.component.scss']
})
export class HrLeaveRequestsViewTableComponent implements OnInit {

  // Item Detail Model
  showModelLoader: boolean = false;
  selecteditem: any = [];
  requestDetails: any = [];
  shortenRequestType: any;
  // Selected Requests
  ...
  getHRDeskLeaveRequestDetails(leaveId) {
    ...
    if(this.viewType === 'leave-shorten'){
      this.hrLeaveRequestsBase.getHRDeskShortenLeaveRequestDetailsByLeaveId(leaveId).subscribe((data: any) => {
  ...
  showViewAcceptShortenModal(selecteditem) {
    this.shortenRequestType = 'acceptShortenLeave';
    this.requestDetails = selecteditem;
    this.getHRDeskLeaveRequestDetails(selecteditem.leave_request_id);
    $('#TableViewShortenModal').modal('show');
  }
  ...
  showViewRejectShortenModal(selecteditem) {
    this.shortenRequestType = 'rejectShortenLeave';
    this.requestDetails = selecteditem;
    this.getHRDeskLeaveRequestDetails(selecteditem.leave_request_id);
    $('#TableViewShortenModal').modal('show');
  }
