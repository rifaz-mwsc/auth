...
@Component({
  selector: 'app-hr-auth-dashboard-remove',
  templateUrl: './hr-auth-dashboard-remove.component.html',
  styleUrls: ['./hr-auth-dashboard-remove.component.scss']
})
export class HrAuthDashboardRemoveComponent implements OnInit {
  ...
  remove(selecteditem) {
    ...
    this.hrAuthorizationService.putHRDeskRemoveAuthorization(selecteditem.dashboard_id).subscribe((data: any) => {
      console.log('Authorization remove data', data);
      setTimeout(() => {
        // Closing model with timeout
        this.closeModel();
      }, this.modelCloseTimeOut);
      this.toastr.warning('Authorization', 'Authorization Removed', { closeButton: true, timeOut: this.toastrTimeOut, progressBar: true, enableHtml: true });
