...
@Component({
  selector: 'app-shiftgroup-manage-employees-view-table',
  templateUrl: './shiftgroup-manage-employees-view-table.component.html',
  styleUrls: ['./shiftgroup-manage-employees-view-table.component.scss']
})
export class ShiftgroupManageEmployeesViewTableComponent implements OnInit {
  ...
  remove(listItem) {
    ...
    this.hrDutyrosterManageShiftgroupBase.getDutyRosterShiftGroupEmployeeRemove(listItem.Id).subscribe((data: any) => {
      console.log('requestList data', data);
      listItem.IsRemoved = data.IsRemoved;
      this.toastr.warning('Employee: ' + data.Name +' ('+data.EmployeeId+' )', 'Employee Removed Successfully', { closeButton: true, timeOut: this.toastrTimeOut, progressBar: true, enableHtml: true });
  ...
  restore(listItem) {
    this.showLoader = true;
    this.hrDutyrosterManageShiftgroupBase.getDutyRosterShiftGroupEmployeeRestore(listItem.Id).subscribe((data: any) => {
      console.log('requestList data', data);
      listItem.IsRemoved = data.IsRemoved;
      this.toastr.success('Employee: ' + data.Name +' ('+data.EmployeeId+' )', 'Employee Restored Successfully', { closeButton: true, timeOut: this.toastrTimeOut, progressBar: true, enableHtml: true });
      this.showLoader = false;
    }, (error: Response | any) => {
      if (error.status === 400) {
        this.generalApiError = error.error.error_message;
